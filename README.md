...
The build is a pure static bundle — no server, no runtime environment
variables required. Any static host will serve it; `vercel.json` configures
Vercel specifically.

```bash
npm ci
npm run verify     # typecheck + lint + build, the same gate CI should run
```

`dist/` is the artifact. Node **20.19+ or 22.12+** is required (see `engines`
and `.nvmrc`); Vite 7 will not run on older releases.

### What `vercel.json` sets

| Concern | Setting |
| --- | --- |
| Hashed assets | `Cache-Control: immutable, max-age=1y` on `/assets/*` — safe because Vite content-hashes every filename |
| `index.html` | left uncached, so a deploy is picked up on the next load |
| CSP | `default-src 'self'` with narrow exceptions, documented below |
| Clickjacking | `frame-ancestors 'none'` |
| MIME sniffing | `X-Content-Type-Options: nosniff` |
| Referrers | `strict-origin-when-cross-origin` |
| Device APIs | camera, microphone and geolocation all denied — none are used |

Three CSP exceptions are deliberate, and each is load-bearing:

- `style-src 'unsafe-inline'` — the HUD positions labels, progress fills and
  swatches through React `style` attributes, which the CSP counts as inline.
- `connect-src https:` — the *optional* AI guide and hosted-voice proxies point
  at an endpoint you own, whose origin is not knowable here. With neither
  configured the app makes no network requests at all after load.
- `media-src blob:` — hosted voice audio arrives as a blob URL.

### Link previews

`public/og.png` is the 1280×720 preview card, regenerated by
`scripts/generate-og.py` (Pillow + fontTools; reads the brand fonts straight out
of `node_modules/@fontsource`, so the card cannot drift from the shipped type).
It is committed, so a deploy never depends on Python being present.

`og:image` must be absolute for crawlers to resolve it. The `absoluteOgUrls`
plugin in `vite.config.ts` stamps the origin at build time from `SITE_ORIGIN`,
falling back to Vercel's `VERCEL_PROJECT_PRODUCTION_URL`. On a host that sets
neither, set `SITE_ORIGIN` yourself:

```bash
SITE_ORIGIN=https://your-domain.example npm run build
```

Left unset, the tag stays relative and the preview card simply shows no image —
a wrong absolute URL would be worse.

### Optional AI guide

Both `VITE_AI_API_URL` and `VITE_TTS_API_URL` are optional and default to off;
see `.env.example`. Neither may hold a provider key — anything `VITE_`-prefixed
is inlined into the bundle and readable by every visitor. Point them at a proxy
you control. Unset, the guide runs fully offline from the project's own data
and the browser's speech engine.
